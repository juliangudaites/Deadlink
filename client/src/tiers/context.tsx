...
export function TierProvider({ children }: { children: ReactNode }) {
  ...
  const [tier, setTier] = useState<TierId>(cached?.tier ?? 'free');
  ...
  const applyResult = useCallback((result: {
    ...
    setTierCache({ tier: result.tier, caps: result.caps, accessCode: result.accessCode, expiresAt: result.expiresAt ?? null });
  ...
  const revokeSession = useCallback(async (deviceId: string) => {
    ...
    if (result.registered) {
      setDeviceLimitOpen(false);
      setPendingCode(null);
    } else if (pendingCode || getStoredAccessCode()) {
      await applyAccessCode(pendingCode || getStoredAccessCode()!);
    }
  ...
  const value = useMemo(
    () => ({
      caps, tier, accessCode, expiresAt, loading, sessions, maxDevices,
      deviceLimitOpen, applyAccessCode, clearAccess, revokeSession, closeDeviceLimit, handleDeviceLimitError,
    }),
    [caps, tier, accessCode, expiresAt, loading, sessions, maxDevices, deviceLimitOpen, applyAccessCode, clearAccess, revokeSession, closeDeviceLimit, handleDeviceLimitError]
  );
