...
import { TIER_CAPS, BURN_TIMER_OPTIONS } from '../utils/tiers.js';
...
function bitcoinPaymentsEnabled() {
  return Boolean(config.bitcoinTipAddress);
}
...
router.get('/catalog', (_req, res) => {
  res.json({
    paymentMethods: { bitcoin: bitcoinPaymentsEnabled(), stripe: isStripeConfigured() },
...
router.post('/redeem', (req, res) => {
  ...
  const resolved = resolveRequestCaps({ headers: { 'x-deadlink-code': code.trim() }, body: {} });
...
router.post('/sessions/revoke', (req, res) => {
  ...
  const resolved = codeFromBody
    ? resolveRequestCaps({ headers: { 'x-deadlink-code': codeFromBody }, body: {} })
    : resolveRequestCaps(req);

  if (!resolved.accessCode || resolved.source === 'free') {
    return res.status(400).json({ error: 'Valid access key required' });
  }
...
router.post('/invoice', invoiceLimiter, async (req, res) => {
  if (!bitcoinPaymentsEnabled()) {
    return res.status(503).json({ error: 'Bitcoin payments not configured', code: 'PAYMENTS_DISABLED' });
  }

  const tier = normalizeTier(req.body?.tier);
  if (!PAID_TIER_IDS.includes(tier)) {
    return res.status(400).json({ error: 'Invalid tier. Choose shadow, void, or spectre.' });
  }
  ...
  const baseBtc = currency === 'BRL' ? brlToBtc(amount, rates.brl) : usdToBtc(amount, rates.usd);
  if (baseBtc <= 0) {
    return res.status(503).json({ error: 'Could not calculate Bitcoin amount' });
  }
  ...
  try {
    const sub = createSubscription({ tier, currency, amountUsd: Math.round(amountUsd * 100) / 100, amountBrl: Math.round(amountBrl * 100) / 100, mode: 'static' });
    ...
    return res.status(201).json({
      ...
      currency,
      rates,
      message: 'Send EXACT BTC amount. Access key unlocks when detected.',
    });
  } catch (err) {
...
router.get('/invoice/:subscriptionId/status', async (req, res) => {
  ...
  if (sub.status === 'paid' && sub.accessCode) {
    return res.json({ subscriptionId: sub.id, status: 'paid', paid: true, tier: sub.tier, tierLabel: TIER_CAPS[sub.tier]?.label, accessCode: sub.accessCode, expiresAt: sub.expiresAt, caps: TIER_CAPS[sub.tier] });
  }
  ...
  if (sub.mode === 'static' && sub.address && sub.amountSats) {
    ...
    if (match) {
      ...
      return res.json({ subscriptionId: paid.id, status: 'paid', paid: true, tier: paid.tier, tierLabel: TIER_CAPS[paid.tier]?.label, accessCode: paid.accessCode, expiresAt: paid.expiresAt, caps: TIER_CAPS[paid.tier] });
    }
  }
  ...
  if (sub.mode === 'stripe' && sub.stripeSessionId && isStripeConfigured()) {
    try {
      ...
      if (session.payment_status === 'paid') {
        const paid = markSubscriptionPaid(sub.id, { stripeSessionId: session.id, stripePaymentIntentId: session.payment_intent?.id || session.payment_intent || null, endorselyReferral: session.metadata?.endorsely_referral || session.client_reference_id || null, affiliateRef: session.metadata?.affiliate_ref || null });
        return res.json({ subscriptionId: paid.id, status: 'paid', paid: true, tier: paid.tier, tierLabel: TIER_CAPS[paid.tier]?.label, accessCode: paid.accessCode, expiresAt: paid.expiresAt, caps: TIER_CAPS[paid.tier] });
      }
    } catch { /* fall through */ }
  }

  res.json({ subscriptionId: sub.id, status: sub.status, paid: false, tier: sub.tier, amountBtc: sub.amountBtc, mode: sub.mode });
});

router.post('/stripe/checkout', invoiceLimiter, async (req, res) => {
  if (!isStripeConfigured()) {
    return res.status(503).json({ error: 'Card payments not configured', code: 'STRIPE_DISABLED' });
  }
  ...
  const rates = await getBtcRates();
  const computedUsd = currency === 'USD' ? amount : (amount / rates.brl) * rates.usd;
  const amountBrl = currency === 'BRL' ? amount : (amount / rates.usd) * rates.brl;
  const endorselyReferral = typeof req.body?.endorselyReferral === 'string' ? req.body.endorselyReferral.trim() : '';
  const affiliateRef = typeof req.body?.affiliateRef === 'string' ? req.body.affiliateRef.trim() : '';

  try {
    const sub = createSubscription({ tier, currency, amountUsd: Math.round(computedUsd * 100) / 100, amountBrl: Math.round(amountBrl * 100) / 100, mode: 'stripe', endorselyReferral: endorselyReferral || null, affiliateRef: affiliateRef || null });
    const baseUrl = getPublicAppUrl(req);
    const session = await createTierCheckoutSession({ subscriptionId: sub.id, tier, tierLabel: TIER_CAPS[tier].label, amountUsd: computedUsd, currency: currency === 'BRL' ? 'brl' : 'usd', endorselyReferral: endorselyReferral || undefined, affiliateRef: affiliateRef || undefined, successUrl: `${baseUrl}/?stripe_session={CHECKOUT_SESSION_ID}`, cancelUrl: `${baseUrl}/?stripe_cancel=1` });
    updateSubscription(sub.id, { stripeSessionId: session.id, checkoutUrl: session.url });
    return res.status(201).json({ mode: 'stripe', subscriptionId: sub.id, tier, tierLabel: TIER_CAPS[tier].label, checkoutUrl: session.url, stripeSessionId: session.id, amountUsd: sub.amountUsd, currency });
  } catch (err) {
    return res.status(500).json({ error: formatStripeError(err), code: err?.code || 'STRIPE_CHECKOUT_FAILED' });
  }
});

router.get('/stripe/complete', async (req, res) => {
  if (!isStripeConfigured()) return res.status(503).json({ error: 'Card payments not configured' });
  ...
  try {
    ...
    if (!sub && session.metadata?.subscriptionId) sub = getSubscriptionById(session.metadata.subscriptionId);
    if (!sub) return res.status(404).json({ error: 'Payment not found' });

    if (session.payment_status === 'paid' && sub.status !== 'paid') {
      sub = markSubscriptionPaid(sub.id, { mode: 'stripe', stripeSessionId: session.id, stripePaymentIntentId: session.payment_intent?.id || session.payment_intent || null, endorselyReferral: session.metadata?.endorsely_referral || session.client_reference_id || null, affiliateRef: session.metadata?.affiliate_ref || null });
    }

    if (sub.status === 'paid' && sub.accessCode) {
      return res.json({ subscriptionId: sub.id, status: 'paid', paid: true, tier: sub.tier, tierLabel: TIER_CAPS[sub.tier]?.label, accessCode: sub.accessCode, expiresAt: sub.expiresAt, caps: TIER_CAPS[sub.tier] });
    }
    return res.json({ subscriptionId: sub.id, status: sub.status, paid: false, paymentStatus: session.payment_status });
  } catch (err) {
